...
$g_continue_check = class_exists ("PDO",/*#ifndef(PHP4) */false/* #*/);
if (defined ("CLASS_direct_dbraw_pdo_mysql")) { $g_continue_check = false; }
if (!defined ("CLASS_direct_db")) { $g_continue_check = false; }

if ($g_continue_check)
{
//c// direct_dbraw_pdo_mysql
/**
...
*             W3C (R) Software License
*/
class direct_dbraw_pdo_mysql extends direct_virtual_class
{
/**
...
------------------------------------------------------------------------- */
	...
	//f// direct_dbraw_pdo_mysql->__construct ()
/**
	* Constructor (PHP5) __construct (direct_dbraw_pdo_mysql)
	*
...
*/
	public function __construct ()
	{
		...
		if (USE_debug_reporting) { direct_debug (5,"sWG/#echo(__FILEPATH__)# -db_class->__construct (direct_dbraw_pdo_mysql)- (#echo(__LINE__)#)"); }
		...
		if (isset ($direct_settings['db_synchronisation']))
		{
			...
			{
				if (!$direct_globals['basic_functions']->include_file ($direct_settings['path_system']."/functions/swg_dbsync.php",1)) { trigger_error ("sWG/#echo(__FILEPATH__)# _dbraw_pdo_mysql_ (#echo(__LINE__)#) reporting: Manual synchronisation trigger unavailable",E_USER_WARNING); }
			}
		}
...
"timeout" => PDO::ATTR_TIMEOUT
	...
	//f// direct_dbraw_pdo_mysql->__destruct ()
/**
	* Destructor (PHP5) __destruct (direct_dbraw_pdo_mysql)
	*
	* @since v0.1.00
*/
	public function __destruct () { $this->disconnect (); }

	//f// direct_dbraw_pdo_mysql->connect ()
/**
	* Opens the connection to a database server and selects a database.
	*
	* @uses   direct_dbraw_pdo_mysql::secure()
	* @uses   direct_debug()
	* @uses   USE_debug_reporting
...
*/
	public function connect ()
	{
		...
		elseif (strlen ($direct_settings['db_dsa']))
		{
			...
			try
			{
				if ($direct_settings['db_peristent']) { $this->resource = new PDO ($direct_settings['db_dsa'],$direct_settings['db_username'],$direct_settings['db_password'],(array ($this->pdo['persistent'] => true,$this->pdo['errormode'] => $this->pdo['errormode_silent'],$this->pdo['timeout'] => $direct_settings['timeout_core']))); }
				else { $this->resource = new PDO ($direct_settings['db_dsa'],$direct_settings['db_username'],$direct_settings['db_password'],(array ($this->pdo['persistent'] => false,$this->pdo['errormode'] => $this->pdo['errormode_silent'],$this->pdo['timeout'] => $direct_settings['timeout_core']))); }
			}
			...
			if (is_object ($this->resource))
			{
				if ((isset ($direct_settings['db_charset']))&&(strlen ($direct_settings['db_charset'])))
				{
					...
					$this->resource->exec ("SET CHARACTER SET ".$direct_settings['db_charset']);
				}
	...
	//f// direct_dbraw_pdo_mysql->disconnect ()
/**
...
*/
	public function disconnect ()
	{
		...
		return /*#ifdef(DEBUG):direct_debug (7,"sWG/#echo(__FILEPATH__)# -db_class->disconnect ()- (#echo(__LINE__)#)",:#*/is_object ($this->resource)/*#ifdef(DEBUG):,true):#*/;
	}

	//f// direct_dbraw_pdo_mysql->query_build ($f_data)
/**
	* Builds a valid SQL query for MySQL and executes it.
	*
	* @param  array $f_data Array containing query specific information.
	* @uses   direct_dbraw_pdo_mysql::query_build_attributes()
	* @uses   direct_dbraw_pdo_mysql::query_build_row_conditions_walker()
	* @uses   direct_dbraw_pdo_mysql::query_build_search_conditions()
	* @uses   direct_dbraw_pdo_mysql::query_build_set_attributes()
	* @uses   direct_dbraw_pdo_mysql::query_build_ordering()
	* @uses   direct_dbraw_pdo_mysql::query_build_values()
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_build_attributes ($f_attributes_array)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_build_ordering ($f_ordering_list)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_build_row_conditions_walker ($f_requirements_array)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_build_search_conditions ($f_conditions_array)
/**
...
------------------------------------------------------------------------- */
	...
	//f// direct_dbraw_pdo_mysql->query_build_set_attributes ($f_attributes_array)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_build_values ($f_values_array)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->query_exec ($f_answer,$f_query)
/**
...
*/
		...
		if (is_object ($this->resource))
		{
			...
			else
			{
				...
				if (($f_answer == "co")||(!$f_result_object)||(!is_object ($f_result_object)))
				{
					...
					else { trigger_error ("sWG/#echo(__FILEPATH__)# -db_class->query_exec ()- (#echo(__LINE__)#) reporting: ANSI ".($this->resource->errorCode ()),E_USER_ERROR); }
				}
				elseif ($f_answer == "ma") { $f_return = $f_result_object->fetchAll ($this->pdo['fetch_assoc']); }
				elseif ($f_answer == "ms")
				{
					$f_return = array ();
				...
				elseif ($f_answer == "sa") { $f_return = $f_result_object->fetch ($this->pdo['fetch_assoc']); }
				elseif ($f_answer == "ss")
				{
	...
	//f// direct_dbraw_pdo_mysql->optimize ($f_table)
/**
...
*/
	public function optimize ($f_table)
	{
		...
		if (is_object ($this->resource)) { $f_return = $this->query_exec ("co","OPTIMIZE TABLE ".$f_table); }
	...
	//f// direct_dbraw_pdo_mysql->secure (&$f_data)
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->transaction_begin ()
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->transaction_commit ()
/**
...
*/
	...
	//f// direct_dbraw_pdo_mysql->transaction_rollback ()
/**
...
define ("CLASS_direct_dbraw_pdo_mysql",true);
