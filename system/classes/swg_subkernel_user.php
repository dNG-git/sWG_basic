...
------------------------------------------------------------------------- */
		...
		if (!direct_class_function_check ($direct_globals['kernel'],"v_user_init"))
		{
			$direct_globals['kernel']->v_call_set ("v_user_init",$this,"user_init");
...
*/
	/*#ifndef(PHP4) */public /* #*/function user_init ($f_threshold_id = "")
	{
		...
		{
			if (!$direct_globals['basic_functions']->include_file ($direct_settings['path_system']."/classes/dhandler/swg_user.php",1)) { $f_return = false; }
		}

		if ($f_return)
		{
			$direct_globals['kernel']->v_call_set ("v_user_check",$this,"user_check");
			$direct_globals['kernel']->v_call_set ("v_user_get",$this,"user_get");
			$direct_globals['kernel']->v_call_set ("v_user_parse",$this,"user_parse");
			$direct_globals['kernel']->v_call_set ("v_user_insert",$this,"user_insert");
			$direct_globals['kernel']->v_call_set ("v_user_update",$this,"user_update");
			$direct_globals['kernel']->v_call_set ("v_user_write_kernel",$this,"user_write_kernel");
			$direct_globals['kernel']->v_call_set ("v_usertype_get_int",$this,"usertype_get_int");
			...
			$f_uuid_data = ((direct_class_function_check ($direct_globals['kernel'],"v_uuid_get")) ? $direct_globals['kernel']->v_uuid_get ("s") : "");
				...
				if ($f_uuid_array)
				{
					if ($direct_globals['kernel']->v_user_check ($f_uuid_array['userid']))
					{
						if (direct_class_function_check ($direct_globals['kernel'],"v_uuid_write"))
						{
							...
							$direct_globals['kernel']->v_uuid_write ($f_uuid_data);
							$direct_globals['kernel']->v_uuid_cookie_save ();
						}

						$f_user_array = $direct_globals['kernel']->v_user_get ($f_uuid_array['userid']);
...
------------------------------------------------------------------------- */
						...
						$direct_globals['kernel']->v_user_write_kernel ($direct_settings['user']['id']);
					}
					else
					{
						...
						if ($direct_globals['kernel']->v_uuid_check_usage ())
						{
							$direct_globals['kernel']->v_uuid_write ($f_uuid_data);
							$direct_globals['kernel']->v_uuid_cookie_save ();
						}
...
*/
	/*#ifndef(PHP4) */public /* #*/function user_write_kernel ($f_user_id)
	{
		...
		if (isset ($direct_globals['db']))
		{
			$direct_globals['db']->init_update ($direct_settings['users_table']);

			$f_user_ip = ($direct_settings['swg_ip_save2db'] ? $direct_settings['user_ip'] : "unknown");

$f_data = ("<sqlvalues>
".($direct_globals['db']->define_set_attributes_encode ($direct_settings['users_table'].".ddbusers_lang",$direct_settings['lang'],"string"))."
".($direct_globals['db']->define_set_attributes_encode ($direct_settings['users_table'].".ddbusers_theme",$direct_settings['theme'],"string"))."
".($direct_globals['db']->define_set_attributes_encode ($direct_settings['users_table'].".ddbusers_lastvisit_ip",$f_user_ip,"string"))."
".($direct_globals['db']->define_set_attributes_encode ($direct_settings['users_table'].".ddbusers_lastvisit_time",$direct_cachedata['core_time'],"string"))."
</sqlvalues>");

			$direct_globals['db']->define_set_attributes ($f_data);

			$f_data = "<sqlconditions>".($direct_globals['db']->define_row_conditions_encode ($direct_settings['users_table'].".ddbusers_id",$f_user_id,"string"))."</sqlconditions>";
			$direct_globals['db']->define_row_conditions ($f_data);
...
$direct_globals['@names']['kernel_user'] = "direct_kernel_user";
