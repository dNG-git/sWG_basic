...
----------------------------------------------------------------------------
$Id: swg_dbraw_mysql.php,v 1.5 2008/12/22 16:20:12 s4u Exp $
...
* servers. This one is designed for MySQL.
*
...
if ($g_continue_check)
{
//c// direct_dbraw_mysql
/**
* This class has been designed to communicate with MySQL servers.
*
* @author     direct Netware Group
* @copyright  (C) direct Netware Group - All rights reserved
* @package    sWG_basic
...
*             W3C (R) Software License
*/
class direct_dbraw_mysql extends direct_virtual_class
{
/**
	* @var string $query_cache This variable saves a built SQL query
*/
	/*#ifndef(PHP4) */protected/* #*//*#ifdef(PHP4):var:#*/ $query_cache;
/**
...
------------------------------------------------------------------------- */
	...
	//f// direct_dbraw_mysql->__construct () and direct_dbraw_mysql->direct_dbraw_mysql ()
/**
	* Constructor (PHP5) __construct (direct_dbraw_mysql)
	*
...
/*#ifdef(PHP4):
/**
	* Constructor (PHP4) direct_dbraw_mysql (direct_dbraw_mysql)
	*
	* @since v0.1.00
*\/
	function direct_dbraw_mysql () { $this->__construct (); }
:#\n*/
	//f// direct_dbraw_mysql->__destruct ()
/**
	* Destructor (PHP5) __destruct (direct_dbraw_mysql)
	*
	* @since v0.1.00
*/
	/*#ifndef(PHP4) */public /* #*/function __destruct () { $this->disconnect (); }

	//f// direct_dbraw_mysql->connect ()
/**
	* Opens the connection to a database server and selects a database.
	*
	* @uses   direct_dbraw_mysql::secure()
	* @uses   direct_debug()
	* @uses   USE_debug_reporting
...
*/
	/*#ifndef(PHP4) */public /* #*/function connect ()
	{
		...
		elseif ((function_exists ("mysql_pconnect"))&&(strlen ($direct_settings['db_dbname'])))
		{
			...
			{
				@stream_set_timeout ($this->resource,$direct_settings['timeout_core']);

				if (mysql_select_db ($direct_settings['db_dbname'],$this->resource))
				{
					if ((isset ($direct_settings['db_charset']))&&(strlen ($direct_settings['db_charset'])))
					{
						$this->secure ($direct_settings['db_charset']);
						mysql_query ("SET CHARACTER SET '$direct_settings[db_charset]'",$this->resource);
					}

					$f_return = true;
				}
				else { trigger_error ("sWG/#echo(__FILEPATH__)# -db_class->connect ()- (#echo(__LINE__)#) reporting: ".(mysql_error ($this->resource)),E_USER_ERROR); }
			}
			else { trigger_error ("sWG/#echo(__FILEPATH__)# -db_class->connect ()- (#echo(__LINE__)#) reporting: Wrong password for the specified username",E_USER_ERROR); }
		}

		return /*#ifdef(DEBUG):direct_debug (7,"sWG/#echo(__FILEPATH__)# -db_class->connect ()- (#echo(__LINE__)#)",:#*/$f_return/*#ifdef(DEBUG):,true):#*/;
	}

	//f// direct_dbraw_mysql->disconnect ()
/**
...
*/
	...
	//f// direct_dbraw_mysql->query_build ($f_data)
/**
	* Builds a valid SQL query for MySQL and executes it.
	*
	* @param  array $f_data Array containing query specific information.
	* @uses   direct_dbraw_mysql::query_build_attributes()
	* @uses   direct_dbraw_mysql::query_build_row_conditions_walker()
	* @uses   direct_dbraw_mysql::query_build_search_conditions()
	* @uses   direct_dbraw_mysql::query_build_set_attributes()
	* @uses   direct_dbraw_mysql::query_build_ordering()
	* @uses   direct_dbraw_mysql::query_build_values()
...
*/
	...
	//f// direct_dbraw_mysql->query_build_attributes ($f_attributes_array)
/**
...
*/
	...
	//f// direct_dbraw_mysql->query_build_ordering ($f_ordering_list)
/**
...
*/
	...
	//f// direct_dbraw_mysql->query_build_row_conditions_walker ($f_requirements_array)
/**
...
*/
	...
	{
		if (USE_debug_reporting) { direct_debug (5,"sWG/#echo(__FILEPATH__)# -db_class->query_build_row_conditions_walker (+f_requirements)- (#echo(__LINE__)#)"); }
			...
			foreach ($f_requirements_array as $f_requirement_array)
			{
				if (is_array ($f_requirement_array))
				{
					...
					elseif ($f_requirement_array['value'] != "*")
					{
						...
						elseif ($f_requirement_array['attributes']['type'] == "string") { $f_return .= " {$f_requirement_array['attributes']['operator']} '{$f_requirement_array['value']}'"; }
	...
	//f// direct_dbraw_mysql->query_build_search_conditions ($f_conditions_array)
/**
	* Creates search requests
	*
	* @param  array $f_conditions_array Conditions given as a XML array tree
	* @uses   direct_dbraw_mysql::secure()
	* @uses   direct_debug()
	* @uses   USE_debug_reporting
	* @return string Valid SQL WHERE definition
...
*/
	/*#ifndef(PHP4) */protected /* #*/function query_build_search_conditions ($f_conditions_array)
	{
		...
		if (is_array ($f_conditions_array))
		{
			...
			if (isset ($f_conditions_array['xml.item']))
			{
				if (($f_conditions_array['xml.item']['attributes']['searchtype'] == "advanced")&&($direct_settings['db_advanced_search_supported'])) { $f_search_advanced = true; }
				unset ($f_conditions_array['xml.item']);
			}
...
"Test Test1 AND Test2 AND Test3 Test4 Test5 Test6 AND Test7".
Result is: Test "Test1 Test2 Test3" Test4 Test5 "Test6 Test7"
------------------------------------------------------------------------- */
					...
					foreach ($f_words as $f_word)
					{
						...
						else
						{
							...
							{
								$f_and_check = false;
								$f_word_buffer = trim (str_replace ('"',"",$f_word_buffer));
								$this->secure ($f_word_buffer);
								$f_search_term .= '"'.$f_word_buffer.'"';
							}
							else
							{
								$f_word_buffer = trim (str_replace ('"',"",$f_word_buffer));
								if ($f_word_buffer) { $f_search_term .= $f_word_buffer; }
								$f_word_buffer = $f_word;
							}
						}
					}
...
------------------------------------------------------------------------- */
					...
					if ($f_word_buffer) { $f_word_buffer = trim (str_replace ('"',"",$f_word_buffer)); }
...
------------------------------------------------------------------------- */
	...
	//f// direct_dbraw_mysql->query_build_set_attributes ($f_attributes_array)
/**
...
*/
	...
	//f// direct_dbraw_mysql->query_build_values ($f_values_array)
/**
	* Builds the SQL VALUES part of a query.
	*
	* @param  array $f_values_array Values definition of the given as a XML
	*         array tree
	* @uses   direct_dbraw_mysql::query_build_values()
	* @uses   direct_debug()
	* @uses   USE_debug_reporting
...
*/
	...
	//f// direct_dbraw_mysql->query_exec ($f_answer,$f_query)
/**
...
*/
	/*#ifndef(PHP4) */public /* #*/function query_exec ($f_answer,$f_query)
	{
		...
		if (is_resource ($this->resource))
		{
			$f_result_pointer = mysql_query ($f_query,$this->resource);

			if ($f_answer == "ar") { $f_return = mysql_affected_rows ($this->resource); }
			elseif ((!$f_result_pointer)||(!is_resource ($f_result_pointer)))
			{
				if ($f_result_pointer) { $f_return = true; }
				else { trigger_error ("sWG/#echo(__FILEPATH__)# -db_class->query_exec ()- (#echo(__LINE__)#) reporting: ".(mysql_error ($this->resource)),E_USER_ERROR); }
			}
			...
			elseif ($f_answer == "ma")
			{
				...
				while ($f_row_array = mysql_fetch_assoc ($f_result_pointer))
				{
					if (!empty ($f_row_array)) { $f_return[] = $f_row_array; }
				}
			}
			elseif ($f_answer == "ms")
			{
				...
				while ($f_row_array = mysql_fetch_row ($f_result_pointer))
				{
					if (!empty ($f_row_array)) { $f_return[] = implode ("\n",$f_row_array); }
				}
			}
			elseif ($f_answer == "nr") { $f_return = mysql_num_rows ($f_result_pointer); }
			elseif ($f_answer == "sa") { $f_return = mysql_fetch_assoc ($f_result_pointer); }
			elseif ($f_answer == "ss")
			{
	...
	//f// direct_dbraw_mysql->optimize ($f_table)
/**
...
*/
	/*#ifndef(PHP4) */public /* #*/function optimize ($f_table)
	{
		...
		if (is_resource ($this->resource)) { $f_return = $this->query_exec ("co","OPTIMIZE TABLE ".$f_table); }
	...
	//f// direct_dbraw_mysql->secure (&$f_data)
/**
...
*/
	/*#ifndef(PHP4) */public /* #*/function secure (&$f_data)
	{
		...
		if (is_resource ($this->resource))
		{
			if (is_array ($f_data))
			{
				foreach ($f_data as $f_key => $f_value) { $f_data[$f_key] = mysql_real_escape_string ($f_value,$this->resource); }
			}
			else { $f_data = mysql_real_escape_string ($f_data,$this->resource); }
		}
	}
...
define ("CLASS_direct_dbraw_mysql",true);

//j// Script specific commands

if (!isset ($direct_settings['db_advanced_search_supported'])) { $direct_settings['db_advanced_search_supported'] = true; }
if (!isset ($direct_settings['db_transaction_supported'])) { $direct_settings['db_transaction_supported'] = false; }
