...
case "form-save":
{
	...
	if ($direct_settings['user']['type'] = "gt")
	{
		if ($g_mode_save)
		{
			if ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_registration_form_save","pre_module_service_action_ajax"); }
			else { $direct_globals['output']->related_manager ("account_registration_form_save","pre_module_service_action"); }
		}
		elseif ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_registration_form","pre_module_service_action_ajax"); }
		else
		{
			$direct_globals['output']->related_manager ("account_registration_form","pre_module_service_action");
			$direct_globals['kernel']->service_https ($direct_settings['account_https_registration'],$direct_cachedata['page_this']);
		}
...
------------------------------------------------------------------------- */
		...
		$direct_globals['formbuilder']->entry_add_password (array ("name" => "apassword","title" => (direct_local_get ("core_password")),"required" => true,"min" => $direct_settings['users_password_min'],"max" => 0,"helper_text" => ((direct_local_get ("core_helper_password_1")).$direct_settings['users_password_min'].(direct_local_get ("core_helper_password_2")))),"2_tmd5");
		$direct_globals['formbuilder']->entry_add_file_ftg (array ("name" => "aregister_tou","title" => (direct_local_get ("account_register_tou")),"size" => "l"),$direct_settings['path_data']."/settings/swg_account_tou.ftf");
		$direct_globals['formbuilder']->entry_add_select (array ("name" => "atou","title" => (direct_local_get ("account_register_tou_accept")),"required" => true,"size" => "s"));
...
<element1 attribute='ddbusers_deleted' value='0' type='string' />
			...
			elseif ($direct_globals['kernel']->v_user_check ("",$direct_cachedata['i_ausername'],true)) { $direct_globals['output']->output_send_error ("standard","account_username_exists","","SERVICE ERROR: &quot;$direct_cachedata[i_ausername]&quot; has already been registered","sWG/#echo(__FILEPATH__)# _a=form-save_ (#echo(__LINE__)#)"); }
			elseif (strlen ($g_username_invalid_chars)) { $direct_globals['output']->output_send_error ("standard","account_username_invalid","SERVICE ERROR: Allowed characters are: 0-9, a-z, A-Z as well as !$%&amp;/(){}[]?@*~#,.-;_ and space","sWG/#echo(__FILEPATH__)# _a=form-save_ (#echo(__LINE__)#)"); }
			elseif (preg_match ("#".(implode ("$|",$g_email_blacklist_array))."$#im",$direct_cachedata['i_aemail'])) { $direct_globals['output']->output_send_error ("standard","account_email_blacklisted","SERVICE ERROR: Blacklisted by the administration","sWG/#echo(__FILEPATH__)# _a=form-save_ (#echo(__LINE__)#)"); }
 ...
 "account_sendmail_title" => direct_local_get ("account_title_registration","text")
					...
					if ($g_continue_check)
					{
						...
						if ($g_mode_ajax_dialog)
						{
							...
							$direct_globals['output']->related_manager ("account_registration_form_save","post_module_service_action_ajax");
						...
						else
						{
							...
							$direct_globals['output']->related_manager ("account_registration_form_save","post_module_service_action");
							$direct_globals['output']->oset ("default","done");
							$direct_globals['output']->output_send ($direct_cachedata['output_job']);
						}
					}
				}
				else { $direct_globals['output']->output_send_error ("standard","core_unknown_error","FATAL ERROR: tmpStorager has reported an error","sWG/#echo(__FILEPATH__)# _a=form-save_ (#echo(__LINE__)#)"); }
			}
			else { $direct_globals['output']->output_send_error ("standard","account_tou_required","","sWG/#echo(__FILEPATH__)# _a=form-save_ (#echo(__LINE__)#)"); }
		}
...
------------------------------------------------------------------------- */
			...
			$direct_globals['output']->header (false,true,$direct_settings['p3p_url'],$direct_settings['p3p_cp']);
			$direct_globals['output']->related_manager ("account_registration_form","post_module_service_action");
	...
	else { $direct_globals['output']->output_send_error ("standard","core_service_inactive","","sWG/#echo(__FILEPATH__)# _a={$direct_settings['a']}_ (#echo(__LINE__)#)"); }
	}
