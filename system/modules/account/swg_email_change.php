...
----------------------------------------------------------------------------
$Id: swg_email_change.php,v 1.7 2008/12/22 15:03:37 s4u Exp $
...
case "form-save":
{
	...
	{
	if (($direct_settings['user']['type'] == "ex")||($direct_settings['user']['type'] == "gt")) { $direct_classes['error_functions']->error_page ("login","core_access_denied","sWG/#echo(__FILEPATH__)# _a={$direct_settings['a']}_ (#echo(__LINE__)#)"); }
	else
	{
	//j// BOA
	if ($g_mode_save) { direct_output_related_manager ("account_email_change_form_save","pre_module_service_action"); }
	else
	{
		direct_output_related_manager ("account_email_change_form","pre_module_service_action");
		$direct_classes['kernel']->service_https ($direct_settings['account_https_email_change'],$direct_cachedata['page_this']);
	}
	...
	if ($g_mode_save) { $direct_cachedata['i_aemail'] = (isset ($GLOBALS['i_aemail']) ? ($direct_classes['basic_functions']->inputfilter_basic ($GLOBALS['i_aemail'])) : ""); }
	else
	{
...
------------------------------------------------------------------------- */
	...
	if (($g_mode_save)&&($direct_classes['formbuilder']->check_result))
	{
		...
		{
			if ($g_target_url) { $direct_classes['output']->redirect (direct_linker ("url1",(str_replace ("[oid]","auid_d+{$direct_settings['user']['id']}++",$g_target_url)),false)); }
...
All rights reserved");
					...
					direct_output_related_manager ("account_email_change_form_save","post_module_service_action");
	...
	else
	{
		...
		direct_output_related_manager ("account_email_change_form","post_module_service_action");
