...
case "select-save":
{
	...
	if ($g_mode_save)
	{
		if ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_language_select_save","pre_module_service_action_ajax"); }
		else { $direct_globals['output']->related_manager ("account_language_select_save","pre_module_service_action"); }
	}
	elseif ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_language_select","pre_module_service_action_ajax"); }
	else
	{
		$direct_globals['output']->related_manager ("account_language_select","pre_module_service_action");
		$direct_globals['kernel']->service_https ($direct_settings['account_https_language'],$direct_cachedata['page_this']);
	}
	...
	if (!$g_mode_save)
	{
		...
		$direct_globals['formbuilder']->entry_add_radio (array ("name" => "alang","title" => (direct_local_get ("account_language")),"required" => true));
	...
	if ($g_mode_save)
	{
		...
		if ($g_mode_ajax_dialog)
		{
			...
			$direct_globals['output']->related_manager ("account_language_select_save","post_module_service_action_ajax");
		...
		else
		{
			...
			$direct_globals['output']->related_manager ("account_language_select_save","post_module_service_action");
...
------------------------------------------------------------------------- */
		...
		$direct_globals['output']->header (false,true,$direct_settings['p3p_url'],$direct_settings['p3p_cp']);
		$direct_globals['output']->related_manager ("account_language_select","post_module_service_action");
