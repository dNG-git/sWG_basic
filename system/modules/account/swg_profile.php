...
case "edit-save":
{
	...
	if ((($direct_settings['user']['type'] == "ex")||($direct_settings['user']['type'] == "gt"))||(($direct_settings['user']['id'] != $direct_cachedata['output_uid'])&&($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) < 4)&&(!$direct_globals['kernel']->v_group_user_check_right ("account_profile_edit")))) { $direct_globals['output']->output_send_error ("login","core_access_denied","","sWG/#echo(__FILEPATH__)# _a={$direct_settings['a']}_ (#echo(__LINE__)#)"); }
	else
	{
	//j// BOA
	...
	if ($g_mode_save)
	{
		if ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form_save","pre_module_service_action_ajax"); }
		else { $direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form_save","pre_module_service_action"); }
	}
	elseif ($g_mode_ajax_dialog) { $direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form","pre_module_service_action_ajax"); }
	else
	{
		$direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form","pre_module_service_action");
		$direct_globals['kernel']->service_https ($direct_settings['account_https_profile_edit'],$direct_cachedata['page_this']);
	}
	...
	if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) < 4) { $direct_globals['output']->options_insert (1,"servicemenu","m=account;s=email_change",(direct_local_get ("account_email_change")),$direct_settings['serviceicon_account_email_change'],"url0"); }
	$direct_globals['output']->options_insert (1,"servicemenu","m=account;s=password_change",(direct_local_get ("account_password_change")),$direct_settings['serviceicon_account_password_change'],"url0");
...
------------------------------------------------------------------------- */
		...
		if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3)
		{
			...
			$direct_cachedata['i_atitle'] = (isset ($GLOBALS['i_atitle']) ? ($direct_globals['basic_functions']->inputfilter_basic ($GLOBALS['i_atitle'])) : "");
			$direct_cachedata['i_aemail'] = (isset ($GLOBALS['i_aemail']) ? ($direct_globals['basic_functions']->inputfilter_basic ($GLOBALS['i_aemail'])) : "");
		}
		else
		{
			if ($direct_settings['account_profile_username_change']) { $direct_cachedata['i_ausername'] = (isset ($GLOBALS['i_ausername']) ? ($direct_globals['basic_functions']->inputfilter_basic ($GLOBALS['i_ausername'])) : ""); }
		...
		$direct_cachedata['i_asignature'] = (isset ($GLOBALS['i_asignature']) ? ($direct_globals['basic_functions']->inputfilter_basic ($GLOBALS['i_asignature'])) : "");
	...
	else
	{
		...
		$direct_cachedata['i_atimezone'] = $g_user_array['ddbusers_timezone'];
		$direct_cachedata['i_asignature'] = $direct_globals['formtags']->recode_newlines ($direct_globals['output']->smiley_cleanup ($g_user_array['ddbusers_signature']),false);
...
------------------------------------------------------------------------- */
	...
	if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3)
	{
		$direct_globals['formbuilder']->entry_add_text (array ("section" => $g_form_section,"name" => "ausername","title" => (direct_local_get ("core_username")),"required" => true,"size" => "s","min" => $direct_settings['users_min'],"max" => 100,"helper_text" => ((direct_local_get ("core_helper_username_1")).$direct_settings['users_min'].(direct_local_get ("core_helper_username_2")))));
		$direct_globals['formbuilder']->entry_add_rcp_text (array ("section" => $g_form_section,"name" => "atitle","title" => (direct_local_get ("account_usertitle")),"size" => "l","max" => 255));
		$direct_globals['formbuilder']->entry_add_email (array ("section" => $g_form_section,"name" => "aemail","title" => (direct_local_get ("account_email")),"required" => true,"size" => "l","min" => 5,"max" => 255));
	}
	else
	{
		if ($direct_settings['account_profile_username_change']) { $direct_globals['formbuilder']->entry_add_text (array ("section" => $g_form_section,"name" => "ausername","title" => (direct_local_get ("core_username")),"required" => true,"size" => "s","min" => $direct_settings['users_min'],"max" => 100,"helper_text" => ((direct_local_get ("core_helper_username_1")).$direct_settings['users_min'].(direct_local_get ("core_helper_username_2"))))); }
		else { $direct_globals['formbuilder']->entry_add ("info",array ("section" => $g_form_section,"name" => "ainfo_1","title" => (direct_local_get ("core_username")))); }

		$direct_globals['formbuilder']->entry_add ("info",array ("section" => $g_form_section,"name" => "ainfo_2","title" => (direct_local_get ("account_email"))));
	}

	$direct_globals['formbuilder']->entry_add_select (array ("section" => $g_form_section,"name" => "aemail_public","title" => (direct_local_get ("account_email_public"))));
//	$direct_globals['formbuilder']->entry_add_select ("atimezone",(direct_local_get ("account_timezone")),false,"s");
	$direct_globals['formbuilder']->entry_add_rcp_textarea (array ("section" => $g_form_section,"name" => "asignature","title" => (direct_local_get ("account_signature")),"size" => "s","max" => 65535));
...
------------------------------------------------------------------------- */
	...
	if ($direct_settings['user']['id'] != $direct_cachedata['output_uid'])
	{
		...
		if ($direct_settings['user']['type'] == "ad") { $direct_globals['formbuilder']->entry_add_select (array ("section" => $g_form_section,"name" => "atype","title" => (direct_local_get ("core_usertype")),"required" => true)); }
		$direct_globals['formbuilder']->entry_add_select (array ("section" => $g_form_section,"name" => "alocked","title" => (direct_local_get ("core_usertype_locked"))));
		if ($direct_settings['user']['type'] == "ad") { $direct_globals['formbuilder']->entry_add_select (array ("section" => $g_form_section,"name" => "abanned","title" => (direct_local_get ("core_usertype_banned")))); }
	}
...
------------------------------------------------------------------------- */
		...
		{
			if (($g_user_array['ddbusers_name'] != $direct_cachedata['i_ausername'])&&($direct_globals['kernel']->v_user_check ("",$direct_cachedata['i_ausername'],true))) { $g_continue_check = false; }
		}

		if ($g_username_invalid_chars) { $direct_globals['output']->output_send_error ("standard","account_username_invalid","SERVICE ERROR: Allowed characters are: 0-9, a-z, A-Z as well as !$%&/(){}[]?@*~#,.-;_ and space","sWG/#echo(__FILEPATH__)# _a=edit-save_ (#echo(__LINE__)#)"); }
		elseif ($g_continue_check)
		{
			...
			$g_user_array['ddbusers_signature'] = $direct_globals['output']->smiley_encode ($g_user_array['ddbusers_signature']);

			if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3)
			{
				$g_user_array['ddbusers_title'] = $direct_globals['formtags']->encode ($direct_cachedata['i_atitle'],true);
				$g_user_array['ddbusers_title'] = $direct_globals['output']->smiley_encode ($g_user_array['ddbusers_title']);
...
"username" => $g_user_array['ddbusers_name']
						...
						$direct_globals['kernel']->v_uuid_write (direct_evars_write ($g_uuid_array));
					...
					if ($g_mode_ajax_dialog)
					{
						...
						$direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form_save","post_module_service_action_ajax");
					...
					else
					{
						...
						$direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form_save","post_module_service_action");
						$direct_globals['output']->oset ("default","done");
						$direct_globals['output']->output_send ($direct_cachedata['output_job']);
					}
				}
				else { $direct_globals['output']->output_send_error ("fatal","core_database_error","","sWG/#echo(__FILEPATH__)# _a=edit-save_ (#echo(__LINE__)#)"); }
			}
			else { $direct_globals['output']->output_send_error ("fatal","core_unknown_error","","sWG/#echo(__FILEPATH__)# _a=edit-save_ (#echo(__LINE__)#)"); }
		}
		else { $direct_globals['output']->output_send_error ("standard","account_username_exists","SERVICE ERROR: &quot;$direct_cachedata[i_ausername]&quot; has already been registered","sWG/#echo(__FILEPATH__)# _a=edit-save_ (#echo(__LINE__)#)"); }
	}
...
------------------------------------------------------------------------- */
		...
		$direct_globals['output']->header (false,true,$direct_settings['p3p_url'],$direct_settings['p3p_cp']);
		$direct_globals['output']->related_manager ("account_profile_edit_{$g_user_array['ddbusers_type']}_{$direct_cachedata['output_uid']}_form","post_module_service_action");
...
case "rname":
{
	...
	$g_username = (isset ($direct_settings['dsd']['idata']) ? ($direct_globals['basic_functions']->inputfilter_basic ($direct_settings['dsd']['idata'])) : "");
	...
	if ($g_username)
	{
		$g_user_array = $direct_globals['kernel']->v_user_get ("",$g_username,true);
		...
		else { $direct_globals['output']->output_send_error ("standard","core_username_unknown","","sWG/#echo(__FILEPATH__)# _a=rname_ (#echo(__LINE__)#)"); }
	}
	else { $direct_globals['output']->output_send_error ("standard","core_unsupported_command","","sWG/#echo(__FILEPATH__)# _a=rname_ (#echo(__LINE__)#)"); }
	//j// EOA
	}
...
case "select":
{
	...
	if ($direct_globals['kernel']->service_init_default ())
	{
	if ($direct_settings['account_profile_list'])
	{
	//j// BOA
	$direct_globals['kernel']->service_https ($direct_settings['account_https_profile_select'],$direct_cachedata['page_this']);
...
------------------------------------------------------------------------- */
	...
	if ($g_task_array)
	{
		if ((isset ($g_task_array['uuid']))&&($g_task_array['uuid'] == $g_uuid)&&(!empty ($g_task_array['account_users_marked'])))
		{
			$g_userid = array_shift ($g_task_array['account_users_marked']);

			if ($direct_globals['kernel']->v_user_check ($g_userid,"",true)) { $direct_globals['output']->redirect (direct_linker ("url1",(str_replace ("[oid]","auid+{$g_userid}++",$g_task_array['account_marked_return'])),false)); }
			else { $direct_globals['output']->output_send_error ("standard","core_username_unknown","","sWG/#echo(__FILEPATH__)# _a=select_ (#echo(__LINE__)#)"); }
		}
		else { $direct_globals['output']->output_send_error ("standard","core_tid_invalid","","sWG/#echo(__FILEPATH__)# _a=select_ (#echo(__LINE__)#)"); }
	}
	else
	{
		$g_tid = uniqid ("");
...
"uuid" => $g_uuid
	...
	else { $direct_globals['output']->output_send_error ("standard","core_service_inactive","","sWG/#echo(__FILEPATH__)# _a=select_ (#echo(__LINE__)#)"); }
	}
...
case "view":
{
	...
	$direct_cachedata['output_uid'] = (isset ($direct_settings['dsd']['auid_d']) ? ($direct_globals['basic_functions']->inputfilter_basic ($direct_settings['dsd']['auid_d'])) : "");
	$direct_cachedata['output_uid'] = (isset ($direct_settings['dsd']['auid']) ? ($direct_globals['basic_functions']->inputfilter_basic ($direct_settings['dsd']['auid'])) : $direct_cachedata['output_uid']);

	$direct_cachedata['page_this'] = $direct_settings['ohandler'].";m=account;s=profile;a=view;dsd=auid+".$direct_cachedata['output_uid'];
	$direct_cachedata['page_backlink'] = "m=account;s=profile;a=select";
	...
	if ($g_mode_embedded) { $direct_globals['output']->related_manager ("account_profile_view_{$g_user_array['ddbusers_type']}_".$direct_cachedata['output_uid'],"pre_module_service_action_embedded"); }
	else { $direct_globals['output']->related_manager ("account_profile_view_{$g_user_array['ddbusers_type']}_".$direct_cachedata['output_uid'],"pre_module_service_action"); }

	$direct_globals['kernel']->service_https ($direct_settings['account_https_profile_view'],$direct_cachedata['page_this']);
	$direct_globals['basic_functions']->require_file ($direct_settings['path_system']."/classes/swg_formtags.php");
	$direct_globals['basic_functions']->require_file ($direct_settings['path_system']."/functions/swg_mods_support.php");
	direct_local_integration ("account");
	...
	if ($g_user_array)
	{
		...
		if (!$g_mode_embedded)
		{
			...
			if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']))
			{
				if ($direct_cachedata['output_uid'] == $direct_settings['user']['id']) { $direct_globals['output']->options_insert (1,"servicemenu","m=account;s=profile;a=edit",(direct_local_get ("account_profile_edit")),$direct_settings['serviceicon_account_profile_edit'],"url0"); }
				elseif (($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3)||($direct_globals['kernel']->v_group_user_check_right ("account_profile_edit"))) { $direct_globals['output']->options_insert (1,"servicemenu","m=account;s=profile;a=edit;dsd=auid+".$direct_cachedata['output_uid'],(direct_local_get ("account_profile_edit")),$direct_settings['serviceicon_account_profile_edit'],"url0"); }
			}
		}

		$g_user_parsed_array = $direct_globals['kernel']->v_user_parse ($direct_cachedata['output_uid']);
		...
		if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3) { $direct_cachedata['output_registration_ip'] = ((($g_user_array['ddbusers_registration_ip'])&&($g_user_array['ddbusers_registration_ip'] != "unknown")) ? direct_html_encode_special ($g_user_array['ddbusers_registration_ip']) : direct_local_get ("core_unknown")); }
		...
		if ($direct_globals['kernel']->v_usertype_get_int ($direct_settings['user']['type']) > 3) { $direct_cachedata['output_lastvisit_ip'] = ((($g_user_array['ddbusers_lastvisit_ip'])&&($g_user_array['ddbusers_lastvisit_ip'] != "unknown")) ? direct_html_encode_special ($g_user_array['ddbusers_lastvisit_ip']) : direct_local_get ("core_unknown")); }
		...
		if ($g_mode_embedded)
		{
			$direct_globals['output']->related_manager ("account_profile_view_{$g_user_array['ddbusers_type']}_".$direct_cachedata['output_uid'],"post_module_service_action_embedded");
			$direct_globals['output']->oset ("account_embedded","view");
		}
		else
		{
			$direct_globals['output']->related_manager ("account_profile_view_{$g_user_array['ddbusers_type']}_".$direct_cachedata['output_uid'],"post_module_service_action");
			$direct_globals['output']->oset ("account","view");
		}

		$direct_globals['output']->output_send (direct_local_get ("account_profile_view"));
	}
	else { $direct_globals['output']->output_send_error ("standard","core_username_unknown","","sWG/#echo(__FILEPATH__)# _a=view_ (#echo(__LINE__)#)"); }
	//j// EOA
	}
